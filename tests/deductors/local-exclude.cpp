...
TEST_CASE("LocalExcludeDeductor class", "[local-exclude-deductor]") {
    ...
    SECTION("deduce room") {
        REQUIRE_FALSE(notes[Bot::WHITE][Bot::BEDROOM].dealt);
        ...
        log.addSuggestion(Bot::WHITE, Bot::Suggestion(Bot::SCARLET, Bot::CANDLESTICK, Bot::BEDROOM));
        log.addShow(Bot::WHITE);

        // check if deduction was made
        REQUIRE(deductor.run(log, notes));
        // check if correct flags has been set
        REQUIRE(notes[Bot::WHITE][Bot::BEDROOM].dealt);
    ...
    SECTION("deduce weapon") {
        REQUIRE_FALSE(notes[Bot::WHITE][Bot::KNIFE].dealt);
        ...
        log.addSuggestion(Bot::WHITE, Bot::Suggestion(Bot::PLUM, Bot::KNIFE, Bot::BATHROOM));
        log.addShow(Bot::WHITE);

        // check if deduction was made
        REQUIRE(deductor.run(log, notes));
        // check if correct flags has been set
        REQUIRE(notes[Bot::WHITE][Bot::KNIFE].dealt);
    ...
    SECTION("deduce player") {
        REQUIRE_FALSE(notes[Bot::WHITE][Bot::PEACOCK].dealt);
        ...
        log.addSuggestion(Bot::WHITE, Bot::Suggestion(Bot::PEACOCK, Bot::LEAD_PIPE, Bot::STUDY));
        log.addShow(Bot::WHITE);

        // check if deduction was made
        REQUIRE(deductor.run(log, notes));
        // check if correct flags has been set
        REQUIRE(notes[Bot::WHITE][Bot::PEACOCK].dealt);
